...
O contexto da sessão não deve chegar como um mapa global. `FatosSelecaoAjudaIncognita`
exemplifica uma projeção local e tipada do Modelo da Situação/Solução (verificado em
`src/`, 2026-09-03): ela muda durante a tentativa, enquanto a fotografia do Modelo do
Usuário permanece estável. A fonte de origem deste parágrafo cita também
`FatosSelecaoAjudaPosicionamento` como exemplo equivalente para o posicionamento; essa
classe **não foi encontrada em `src/`** nesta verificação — tratar como projeto ainda
não integrado, não como fato atual, até confirmar.
