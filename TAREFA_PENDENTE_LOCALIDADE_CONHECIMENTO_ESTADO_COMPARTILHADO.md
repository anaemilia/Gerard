...
3. **Bug de UI encontrado durante teste manual, registrado mas não corrigido**: limite superior de arraste de palavra do enunciado (`limiteSuperior = 58 + altura`, em `processarMovimentoArraste`) sobrepõe a barra de ícones Medidas/Relações — ver `RELATORIO_BUG_LIMITE_SUPERIOR_ARRASTE_TEXTO_ENUNCIADO_2026-08-06.md`.
4. **"Estado modificado" — alvo da incógnita recalculado**: encontrado durante o mesmo teste manual — mudar um papel-dado (ex.: Transformação) por outra representação (eixo x) deixava a pergunta de confirmação da incógnita comparando contra o curado original, gerando falso positivo. Implementado e commitado (`d70a5c4`) — `valorDigitadoCorrespondeAoCurado` agora usa `obterValorAlvoParaPapel`, que recalcula o alvo a partir do `Snapshot` atual quando disponível, caindo no curado só quando não há papéis-dado suficientes conhecidos ainda. Ver `RELATORIO_ESTADO_MODIFICADO_ALVO_RECALCULADO_INCOGNITA_2026-08-06.md`. **Não implementado ainda**: um registro/flag de auditoria explícito e visível para pesquisa, distinguindo papéis-dado modificados dos curados independentemente de a incógnita ter sido respondida — fica para quando for pedido.

**Fase B2 literal (`Main.java` manipulando os objetos do piloto diretamente) continua não autorizada e sem decisão tomada** — a conversa não voltou a ela depois do pivô acima.
