...
**Implicação para a Fase B2**: o modelo de "8 origens = 8 tipos de ação do usuário" que o plano original presumia não é exatamente isso. `PROTOCOLO` é, na prática, "correção/simulação do sistema, sem uma origem de UI mais específica" — três usos concretos, nenhum sendo replay de arquivo. Migrar para os objetos do piloto (que têm `OrigemAcao.ORIGEM_SISTEMA` como conceito already-existente e correto para isso) na verdade se encaixa bem aqui — `PROTOCOLO` mapearia naturalmente para `ORIGEM_SISTEMA`, não para uma nova categoria.

**Confirmado (2026-08-06, atualização deste relatório)**: existem três mecanismos de "replay" no repositório, e nenhum deles usa `Origem.PROTOCOLO`:

1. `TesteReplayProtocolosReais.java` — zero referências a `EstadoSemanticoCompartilhado` no arquivo (confirmado por busca direta). Chama os agentes (Monitor/ZDP/Modelador) diretamente, "pula a tela inteiramente" (comentário do próprio `TesteMonkeyGuiadoPorCasosReais.java`, linha 42). Nunca toca o estado compartilhado.
2. `TesteMonkeySemiGuiado.java` — dirige a `TelaGerard` real via `java.awt.Robot`, mas com ações **aleatórias** (stress test). Passa pelos manipuladores de evento reais de `Main.java`, então toca `EstadoSemanticoCompartilhado` — mas via as origens genuínas (`ARRASTE`, `EDICAO_TEXTO` etc.), não `PROTOCOLO`, porque do ponto de vista de `Main.java` um evento de mouse sintetizado pelo `Robot` é indistinguível de um evento real.
3. `TesteMonkeyGuiadoPorCasosReais.java` — o mecanismo mais próximo de "replay de protocolo real de verdade": dirige `TelaGerard` via `Robot` seguindo o roteiro **exato** de um episódio real de `protocolos_reais_replay.tsv`, "exercitando o pipeline inteiro de eventos reais de mouse" (comentário do arquivo). Mesma conclusão do item 2: toca `EstadoSemanticoCompartilhado` pelas origens genuínas de UI, não por `PROTOCOLO`.

Conclusão: **`Origem.PROTOCOLO` nunca é usada para replay, em nenhum dos três mecanismos existentes.** É seguro tratá-la, na Fase B2, como sinônimo de "correção/simulação interna do sistema" (mapeando para `OrigemAcao.ORIGEM_SISTEMA`) — não como uma categoria de replay que precisaria de tratamento especial.
