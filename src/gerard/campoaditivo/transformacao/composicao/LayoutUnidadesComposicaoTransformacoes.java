...
/**
 * Posiciona os quadradinhos no funil de cada uma das três zonas de
 * Composição de Transformações — análogo a
 * {@link gerard.campoaditivo.transformacao.processo.LayoutUnidadesProcessoTransformacao},
 * mas sem o ramo de "estado" (grade retangular): as três zonas são sempre
 * funil, cada uma com sua própria distribuição trapezoidal independente das
 * outras duas.
 */
public final class LayoutUnidadesComposicaoTransformacoes {
    ...
    public List<Rectangle> calcular(List<CirculoVenn> zonas, int indice,
        ...
        GeometriaComposicaoTransformacoes geometria =
                GeometriaComposicaoTransformacoes.calcular(zonas);
        Polygon funil = tipo == TipoProcessoTransformacao.RETIRADA
                ? geometria.getFunilRetirada(indice)
                : geometria.getFunilInsercao(indice);
     ...
     * mesmo algoritmo de
     * {@link gerard.campoaditivo.transformacao.processo.LayoutUnidadesProcessoTransformacao#calcular(List, int, int)},
     * reaplicado por não haver um ponto de extensão compartilhado sem
     * acoplar os dois pacotes um ao outro (a única dependência real é o
     * polígono do funil, já parametrizado por índice aqui).
     */
    ...
    private List<LinhaFunil> criarLinhasDoFunil(Polygon funil,
        ...
        for (int i = 0; i < quantidadeLinhas; i++) {
            ...
            int[] faixa = calcularFaixaHorizontal(funil, y + tamanho / 2.0d);
            ...
            linhas.add(new LinhaFunil(y, (esquerda + direita) / 2, capacidade));
        }
        return linhas;
    }
    ...
    private int[] calcularFaixaHorizontal(Polygon poligono, double y) {
        ...
        return new int[] {(int) Math.ceil(esquerda), (int) Math.floor(direita)};
    }
