...
| `AG_EME` | Exibir mensagem explicativa | Conceitual | Visual | **Implementado (2026-08-16)** — `mostrarDicaOperacaoIncognita` agora explica conceitualmente o papel manipulado (via `CatalogoExplicacoesConceituaisPapel`, chave de explicação carregada pelo objeto rico `DescritorRepresentacaoPapel`/`PapelQuantitativo`), além da instrução operacional já existente (`ui.hint.chooseOperation`). Ver `RELATORIO_ITEM1_AG_EME_EXPLICACAO_CONCEITUAL_2026-08-16.md`. |
| `AG_EMCME` | Exibir material concreto + mensagem explicativa sobre o uso do material | Procedimental | Manipulativa + Visual | **Implementado por completo (2026-08-07, commit `898097e`) — status desta tabela estava desatualizado.** A mensagem existe (`mostrarAvisoLimiteTentativasAtingido`/`ui.notice.attemptLimitReached`); o material concreto (diagrama complementar — quadradinhos/barras/processo) passa a ficar disponível exatamente na 3ª rejeição consecutiva (`Main.deveExibirDiagramaComplementar()` → `escaladaNoLimite` → `SeletorRepresentacaoComplementar.deveExibir`), some de novo ao "Restaurar". O próprio commit já dizia "Resolve AG_EMCME (item 5 do levantamento de pendências) por completo" — confirmado revisitando o código em 2026-08-16, corrigida a tabela que não refletia isso. |
